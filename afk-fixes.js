...
(function () {
  ...
  /* --------------------------------------------------------------------------
   * 修正#8:存檔匯入在 Android 行動模式讀到 0 byte → JSON 解析失敗
   *
   * 問題:原作 importSave 用「未掛進 DOM 的 input.click()」開系統選檔窗(createElement 後
   *   直接 click,從不 appendChild)。Android Chrome 行動模式開選檔窗時會把頁面切到背景
   *   (lifecycle freeze / discard),回前景後這顆 detached 的 <input> 連同它持有的 File blob
   *   可能被回收 → onchange 時 input.files[0] 讀到 0 byte → JSON.parse('') 拋錯 →「JSON 解析失敗」。
   *   切「桌面版網站」模式 / iOS / 桌機都正常:那些情境不會用同一條背景凍結路徑,blob 還活著。
   *   (import 在主選單載入畫面觸發,#game-screen 仍 hidden,故與修正#3 的 visibilitychange 存檔無關。)
   * 解法:只在 Android 行動模式覆寫 importSave —
   *   1) 把 input 真正插進 document(離屏)再 click、讀完才移除 → 元素與 blob 不被 GC,跨背景仍在。
   *   2) 讀取多重後援:先 file.text()(Promise,Android 最穩)→ 退 FileReader.readAsText →
   *      退 readAsArrayBuffer+TextDecoder;任一讀到非空就用。
   *   3) 全讀到空 / file.size===0 → 給「明確、可行動」的訊息(雲端/下載夾選檔常 0 byte,
   *      建議先存到本機再選),而非籠統的「JSON 解析錯誤」。
   *   驗證存檔格式、備份原檔、倉庫(wh)還原詢問、刷新清單等下游邏輯,完全沿用原作流程(忠實鏡射)。
   *   非 Android 不覆寫,維持原作 importSave(本來就正常),把改動的影響面限縮在「唯一會壞的平台」。
   * 何時可移除:原作者把 importSave 改成「input 掛進 DOM + 穩定讀取」時,本段即多餘,可整段刪掉
   *   (抓不到 importSave 自動 no-op)。⚠ 原作者改了匯入的存檔格式/倉庫處理時,本段的下游鏡射要一起跟。
   * ------------------------------------------------------------------------ */
  (function () {
    var isAndroidMobile = /Android/i.test(navigator.userAgent || '');

    function readViaBlobText(file) {
    ...
    function readViaArrayBuffer(file) {
      return new Promise(function (resolve) {
        try {
          var r = new FileReader();
          r.onload = function () {
            try { resolve(new TextDecoder('utf-8').decode(r.result)); } catch (e) { resolve(''); }
          };
          r.onerror = function () { resolve(''); };
    ...
    // 依序試最穩的讀法,任一讀到非空字串就採用;全空才回 ''(視為讀取失敗)
    function readRobust(file) {
      return [readViaBlobText, readViaFileReader, readViaArrayBuffer].reduce(function (p, strat) {
        return p.then(function (acc) {
          if (acc && acc.trim()) return acc;
          return strat(file).then(function (t) { return (t && t.trim()) ? t : acc; });
        });
      }, Promise.resolve(''));
    }

    // 下游:把讀到的存檔文字寫進存檔位 n(忠實鏡射原作 importSave 的驗證/備份/倉庫/刷新流程)
    function processSaveText(text, n) {
      var d;
      ...
      var existing = (typeof window.slotSummary === 'function') ? window.slotSummary(n) : null;
      if (existing && !confirm('存檔 ' + n + ' 已有角色（' + existing.cls + ' Lv.' + existing.lv + ' ' + existing.name + '）。\n確定要用匯入的存檔「取代」它嗎？\n（原存檔會自動備份，可於載入畫面點「復原備份」還原）')) return;
      var whData = d.wh;
      var saveText = text;
      if (whData !== undefined) { var _c = {}; for (var k in d) { if (k !== 'wh') _c[k] = d[k]; } saveText = JSON.stringify(_c); }
      ...
      if (whData !== undefined) {
        ...
        if (typeof window.whKey === 'function' && confirm('此匯入檔包含倉庫資料（物品 ' + _cnt + ' 項、金幣 ' + _gold.toLocaleString() + '）。\n是否一併還原倉庫？\n⚠ 會覆蓋該角色所屬模式（' + ((d.p && d.p.classicMode) ? '經典' : '非經典') + '）的共用倉庫。')) {
          localStorage.setItem(window.whKey(d.p), JSON.stringify({ items: whData.items || [], gold: whData.gold || 0 }));
          whMsg = '\n倉庫已一併還原。';
        } else {
          whMsg = '\n（倉庫維持原狀，未還原）';
        }
      }
      if (typeof window.openSlotSelect === 'function') window.openSlotSelect('load');   // 重新整理清單(匯入只在 load 模式)
      var ns = (typeof window.slotSummary === 'function') ? window.slotSummary(n) : null;
      alert('已匯入到存檔 ' + n + '：' + (ns ? (ns.cls + ' Lv.' + ns.lv + '　' + ns.name) : '完成') + '。' + (cur ? '\n（原存檔已自動備份，可點「復原備份」還原）' : '') + whMsg);
    }

    function importSavePatched(n) {
      var input = document.createElement('input');
      input.type = 'file';
      input.accept = '.json,application/json';
      // 關鍵:把 input 掛進 DOM(離屏)再 click。Android 開選檔窗會把頁面切背景,detached 的 input
      //   連同 File blob 會被回收 → 讀到 0 byte;掛進 DOM 讓元素活著跨過背景。用離屏定位而非 display:none
      //   (部分 Android 對 display:none 的 input 不觸發系統選檔窗)。
      input.style.cssText = 'position:fixed;left:-9999px;top:0;width:1px;height:1px;opacity:0;';
      var done = false;
      function cleanup() { try { if (input.parentNode) input.parentNode.removeChild(input); } catch (e) {} }
      input.addEventListener('change', function () {
        if (done) return; done = true;
        var file = input.files && input.files[0];
        if (!file) { cleanup(); return; }   // 使用者取消選檔
        // 萬一 120s 保險清理已先把 input 移除(使用者在選檔窗停超過 120s 才選檔),讀檔前重新掛回 DOM,
        //   確保整段非同步讀取期間元素仍在 DOM、blob 不被回收(不破壞本修正要修的那條坑)。
        try { if (!input.parentNode) document.body.appendChild(input); } catch (e) {}
        readRobust(file).then(function (text) {
          cleanup();
          if (!text || !text.trim()) {
            alert(file.size === 0
              ? '匯入失敗：選到的檔案是 0 byte（空檔）。\n若是從雲端／下載資料夾選取，請先把存檔下載到手機本機儲存空間，再從「檔案」App 選取。'
              : '匯入失敗：檔案讀取為空，請改從「檔案」App 重新選取存檔檔。');
            return;
          }
          processSaveText(text, n);
        });
      });
      document.body.appendChild(input);
      input.click();
      // 保險:使用者直接關閉選檔窗(部分 Android 不發 change)→ 久未選檔就清掉離屏 input,不留垃圾
      setTimeout(function () { if (!done) cleanup(); }, 120000);
    }

    function install() {
      if (!isAndroidMobile) return true;   // 非 Android:用原作 importSave(本來就正常),不覆寫
      if (typeof window.importSave !== 'function' || window.importSave.__androidImport) return false;
      importSavePatched.__androidImport = true;
      window.importSave = importSavePatched;
      console.log('[AFK-fixes] 匯入存檔 Android 改用 DOM-attached input + 穩定讀取(修手機 0 byte) 已掛上;非 Android 走原版');
      return true;
    }
    ...
    } catch (e) { console.warn('[AFK-fixes] Android 匯入修正 安裝失敗,已略過:', e); }
  })();
