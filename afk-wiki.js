...
(function () {
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '額外傷害・額外魔法點數・最終傷害（三種傷害加成，最容易搞混）', lines: [
      '這三個常被當成同一種，其實是<b>物理固定加值／魔法固定加值／總倍率</b>，作用完全不同：',
      '<b>額外傷害</b>：你<b>每一擊額外加的固定傷害</b>（一般攻擊──近戰／遠程──都吃）。來源：職業每升級成長、裝備、套裝（紅獅 2 件 +5、月光 2 件 +2、暗影 2 件 +7）、勇敢藥水等增益。',
      '<b>額外魔法點數</b>：<b>名字會騙人——它不是 MP（魔力），是「每發魔法／技能直接加上的固定傷害」</b>。主要看<b>智力</b>（智力 11＝+2，逐級到 <b>80 以上封頂 +25</b>，逐級值見「能力值」分頁），另有部分魔杖每強化 +1、祝福的武器 +2、太初詞綴飾品 +2、套裝（紅獅 +3、學徒 +6、魔女 +5）。',
      '<b>「額外魔法點數」和「魔攻」差在哪</b>：魔攻是<b>放大法術本身的倍率</b>（智力越高放大越多），額外魔法點數是<b>直接加一筆固定傷害</b>。兩者算完都還會再被目標魔防減免。',
      '<b>最終傷害 +X%</b>：上面全部加總算完後，再對<b>最終數字整個乘上去</b>（最後一關、最值錢）。例：紅獅 5 件＝幾乎所有物理攻擊 ×1.2；古老的劍／巨劍＝該武器 ×1.5；反擊屏障＝反擊／居合 ×2。'
    ]},
    // ── 二、你打怪：傷害怎麼算（進攻） ──
    { t: '重擊 vs 爆擊（含魔法爆擊，是不一樣的東西）', lines: [
    ...
    { t: '命中：打不打得中（含擦傷、5% 必中）', lines: [
      ...
      '怪的「防禦」數值<b>越低代表越強</b>（越難被你命中）；你自己強化防具、加防禦的詞綴則是讓怪更難打中你、更耐打（你自己「閃避（ER）」是另一回事，見下面「防禦」段）。'
    ]},
    ...
    // ── 三、怪打你：怎麼少受傷（防禦） ──
    { t: '怪打你時：迴避(ER)・傷害減免(DR)・受到傷害減少%（三種減傷，可疊加）', lines: [
      '被怪打中時，傷害會<b>依序</b>經過這幾關，三者機制不同、可同時生效：',
    ...
    { t: '⚔ 經典模式（硬核挑戰）', lines: [
      ...
      '<b>例外</b>：<b>職業限定的試煉任務道具不受這個 ×1/10</b>，照原本機率掉（不然職業試煉在經典模式會難上 10 倍）；其餘一般道具照樣 1/10。',
  ...
  var STATS_SECTIONS = [
    ...
    { t: '🔮 智力（INT）', lines: [
      '魔法<b>傷害、命中、爆擊率</b>與「<b>額外魔法點數</b>」的主要來源，並降低魔法的 <b>MP 消耗</b>。<b>注意：MP 上限不看智力</b>（那是精神＋裝備給的）；「額外魔法點數」也不是 MP，是固定魔法傷害（詳見「戰鬥機制」分頁）。',
