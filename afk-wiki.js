...
(function () {
  ...
  function renderPledge() {
    ...
    out += wCard('💥 打血盟敵人的好處與注意',
      wDesc('對血盟敵人（含攻城血盟兵）<b>你的傷害 ×2</b>（城門／守護塔等建築不算）。') +
      wDesc('掉落：必掉王族搜索狀；野外血盟與攻城敵人<b>額外 1% 機率多掉一件物品</b>（依稀有度隨機，會把非最稀有物品權重加倍）。') +
      wDesc('多掉的若是裝備必帶強化：多落在 +0～安定值（平均）；超出安定值機率 +1：0.1%、+2：0.01%、+3：0.001%、+4：0.0001%；另 <b>1% 帶「祝福的」</b>（屬性／遠古要靠象牙塔碧恩，不會隨機帶）。') +
      wDesc('<b>野外血盟敵人不給經驗、不給金錢</b>（攻城血盟兵給 30×你等級經驗、一樣不給金錢）；HP 低於 20% 時每 3 秒 10% 機率傳送逃走（逃走不算擊殺、拿不到掉落）。'));
