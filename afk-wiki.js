...
(function () {
  ...
  // MAGIC_FACT — 職業魔法「實際數據」金框資料(逐條追 js/ 原始碼查證,2026-07-05)
  // 每 value 是字串陣列,渲染時每條前自動加「・」。只放「遊戲內文字沒講清/與 code 不符」的硬資料。
  var MAGIC_FACT = {
    // ===== 法師魔法(reqM) =====
    sk_heal1: ['回復量＝(20~40)×(1＋3×魔法傷害÷16)，吃智力(魔法傷害)、不吃階級', '只治療自己，不及隊友'],
    sk_shield: ['把自己的防禦(AC) -2(變強)，持續 20 分鐘'],
    sk_lightarrow: ['吃智力(魔法傷害)的單體無屬性攻擊，骰值 1~6 再乘階級係數 ×1.33', 'MP 消耗會隨智力降低(智力 45 以上最多省 30%)，但不隨階級變'],
    sk_icearrow: ['吃智力(魔法傷害)的單體水屬性攻擊，骰值 1~6 乘階級 ×1.33；打地屬怪 ×1.4、被火屬怪剋 ×0.6'],
    sk_windblade: ['吃智力(魔法傷害)的單體風屬性攻擊，骰值 1~6 乘階級 ×1.33；打水屬怪 ×1.4、被地屬怪剋 ×0.6'],
    sk_sunlight: ['讓被動怪主動接近你的延遲從約 5 秒縮到約 1 秒(更快遇敵)，持續 2 小時', '軍王之室、底比斯祭壇內無效'],
    sk_holy_wpn: ['額外傷害 +1、額外命中 +1，持續 20 分鐘'],
    sk_firearrow: ['吃智力(魔法傷害)的單體火屬性攻擊，骰值 1~6 乘階級 ×1.67；打地屬怪 ×1.4、被水屬怪剋 ×0.6'],
    sk_hell_fang: ['吃智力(魔法傷害)的單體地屬性攻擊，骰值 1~6 乘階級 ×1.67；打風屬怪 ×1.4、被火屬怪剋 ×0.6'],
    sk_cold_shiver: ['吃智力(魔法傷害)的單體無屬性攻擊，骰值 1~10 乘階級 ×1.67', '造成傷害同時吸取等量生命(最多補到滿血)'],
    sk_poison_curse: ['純下毒、不造成直接傷害；命中後每 3 秒扣 1~8 點毒傷、持續 15 秒(約 5 跳)', '命中看智力(魔法命中)與雙方等級差、怪魔防，最高 95%'],
    sk_antidote: ['解除中毒；一次只解一人、優先自己再輪傭兵；身上沒中毒就不施放'],
    sk_ench_wpn: ['額外傷害 +2，持續 30 分鐘'],
    sk_reveal: ['讓隱形的史巴托現形、可被攻擊，持續 3 分鐘'],
    sk_load_up: ['負重上限 +50，持續 30 分鐘；效果結束前不能再施放'],
    sk_aurora: ['吃智力(魔法傷害)的全體風屬性攻擊，每隻 2~24 點乘階級 ×2'],
    sk_chill: ['吃智力(魔法傷害)的全體水屬性攻擊，每隻 3~18 點乘階級 ×2'],
    sk_heal_mid: ['回復量＝(51~100)×(1＋3×魔法傷害÷16)，吃智力、不吃階級', '只治療自己'],
    sk_dark_blind: ['純異常、無直接傷害；命中使目標目盲、持續 10 秒', '命中看智力(魔法命中)與等級差、怪魔防，最高 95%'],
    sk_shield2: ['把自己的防禦(AC) -3(變強)，持續 30 分鐘'],
    sk_undead_bane: ['純即死術、不造成傷害；只對不死系非頭目有效，對頭目無效', '成功率最高 60%(依智力魔法命中與等級差、怪魔防浮動)'],
    sk_energy_sense: ['顯示目標的屬性弱點；MP 消耗 8'],
    sk_fireball: ['吃智力(魔法傷害)的全體火屬性攻擊，兩段各 2~12 與 3~18，合計乘階級 ×2.33'],
    sk_rock_prison: ['吃智力(魔法傷害)的全體地屬性攻擊，每隻 4~20 點乘階級 ×2.33'],
    sk_vampire: ['吃智力(魔法傷害)的單體無屬性攻擊，骰值 2~18 乘階級 ×2.33', '造成傷害同時吸取等量生命(最多補到滿血)'],
    sk_break: ['純異常、無直接傷害；命中使目標損壞(更容易被打)，持續 25 秒'],
    sk_slow: ['純異常、無直接傷害；命中使目標緩速，持續 30 秒'],
    sk_dex_up: ['敏捷 +5，持續 20 分鐘；MP 消耗 45'],
    sk_magic_shield: ['完全吸收敵方一次魔法/技能攻擊，觸發後消失並 3 秒內不能再放；最長維持 16 秒', '傭兵不會自動維持這招'],
    sk_meditation: ['MP 自然恢復 +5，持續 10 分鐘；施放消耗 40 點 HP'],
    sk_thunder: ['吃智力(魔法傷害)的單體風屬性攻擊，骰值 8~72 乘階級 ×2.67'],
    sk_ice_spike: ['吃智力(魔法傷害)的單體水屬性攻擊，骰值 6~60 乘階級 ×2.67'],
    sk_mummy_curse: ['純異常、無直接傷害；命中使目標石化 6 秒，對頭目無效'],
    sk_dark_shadow: ['純異常、無直接傷害；命中使目標目盲、持續 20 秒'],
    sk_heal2: ['回復量＝(101~200)×(1＋3×魔法傷害÷16)，吃智力、不吃階級', '只治療自己'],
    sk_holy_light: ['解除石化與麻痺；一次只解一人、優先自己再輪傭兵'],
    sk_charm: ['迷魅非頭目怪成為僕人(攻擊額外加等同魅力的命中與傷害)，成功率最高 60%', '無法迷魅頭目；有召喚精通時對等級低於自己的非頭目怪必定成功'],
    sk_mana_drain: ['消耗 50 點 HP、對怪施放，命中(依智力魔法命中)後吸取 MP＝1~(怪等級÷2)，不造成傷害'],
    sk_earthquake: ['吃智力(魔法傷害)的單體地屬性攻擊，骰值 10~80 乘階級 ×3'],
    sk_blaze: ['吃智力(魔法傷害)的單體火屬性攻擊，骰值 1~100(浮動極大)乘階級 ×3'],
    sk_weaken: ['純異常、無直接傷害；命中使目標弱化，持續 30 秒'],
    sk_haste_spell: ['提升攻擊速度(加速)，持續 20 分鐘；戴風之頭盔時免 MP'],
    sk_str_up: ['力量 +5，持續 20 分鐘；MP 消耗 50'],
    sk_bless_wpn: ['額外傷害 +2、額外命中 +2，持續 20 分鐘'],
    sk_zombie: ['召喚人形殭屍近戰，傷害與命中吃魅力(不吃智力)；每次攻擊段數＝魅力÷6(魅力 60 封頂、最多 10 段)', '每段＝(1~12)＋(魅力÷5×(1＋等級÷20))再扣目標防禦，持續 1 小時', '同時只能存在一個召喚物'],
    sk_cancel: ['解除冰凍/石化/中毒/麻痺/灼傷/燙傷；一次只解一人、優先自己', '只有被沉默或魔法封印時才無法施放'],
    sk_ice_lance: ['吃智力(魔法傷害)的單體水屬性攻擊，骰值 8~80 乘階級 ×3.33', '命中後依智力魔法命中判定冰凍 6 秒，對頭目無效'],
    sk_tornado: ['吃智力(魔法傷害)的全體風屬性攻擊，三段各 2~18，合計乘階級 ×3.33'],
    sk_disease: ['純異常、無直接傷害；命中使目標疾病，持續 30 秒'],
    sk_greater_haste: ['提升攻擊速度(加速)，持續 40 分鐘；戴風之頭盔時免 MP'],
    sk_summon: ['召喚物隨玩家等級自動升級(七種，32/40/52/60/64/72 級各換階，Lv72 黑豹最強)，傷害命中吃魅力(不吃智力)', '每次攻擊段數＝魅力÷6(魅力 60 封頂、最多 10 段)；60 級以上召喚物附帶額外觸發技', '持續 1 小時，同時只能存在一個召喚物'],
    sk_holy_dash: ['迴避 +15，持續 64 秒'],
    sk_regen: ['全隊(自己＋所有傭兵)每 3 秒回一次血、共 5 次(15 秒)；每次＝(31~60)×(1＋3×魔法傷害÷16)，吃智力'],
    sk_quake: ['吃智力(魔法傷害)的全體地屬性攻擊，每隻 4~60 點乘階級 ×3.67'],
    sk_blizzard: ['吃智力(魔法傷害)的全體水屬性攻擊，四段各 2~20，合計乘階級 ×3.67'],
    sk_fire_prison: ['勾在增益欄維持的持續傷害：每 2 秒對全體造成火屬性 (1~15)×(1＋3×魔法傷害÷16)×階級 ×3.67，維持 10 秒(約 5 次)'],
    sk_seal: ['純異常、無直接傷害；命中使目標真空，持續 16 秒'],
    sk_invisible: ['隱身 64 秒；施放消耗 20 點 HP'],
    sk_full_heal: ['回復量＝(153~300)×(1＋3×魔法傷害÷16)，吃智力、不吃階級', '只治療自己'],
    sk_resurrection: ['學會後可在死亡時立即原地復活(不用等 15 秒或復活卷軸)'],
    sk_holy_barrier: ['受到的所有傷害 -30%，持續 32 秒'],
    sk_sleep_mist: ['純異常、無直接傷害；全體命中後沉睡 8 秒，對頭目無效'],
    sk_thunder_storm: ['吃智力(魔法傷害)的全體風屬性攻擊，八段各 1~9，合計乘階級 ×4'],
    sk_fire_storm: ['吃智力(魔法傷害)的全體火屬性攻擊，兩段各 3~36，合計乘階級 ×4'],
    sk_meteor: ['吃智力(魔法傷害)的全體火屬性攻擊，六段各 1~15，合計乘階級 ×4.33(高階級讓小骰值也很痛)'],
    sk_disintegrate: ['吃智力(魔法傷害)的單體無屬性攻擊，骰值 2~140(法師單發骰值最高)乘階級 ×4.33'],
    sk_blizzard_storm: ['勾在增益欄維持的持續傷害：每 4 秒對全體造成水屬性 (1~10)×(1＋3×魔法傷害÷16)×階級 ×4.33，並判定冰凍(魔法命中 -3)、維持 32 秒(約 8 次)'],
    sk_soul_up: ['HP 與 MP 上限各 +20%，持續 20 分鐘'],
    sk_abs_barrier: ['進入與世界隔絕狀態 7 秒：不受任何傷害，但也無法攻擊/施法/用道具/自然恢復'],

    // ===== 妖精專屬(reqE) =====
    sk_elf_mr: ['魔防 +10，持續 20 分鐘'],
    sk_elf_mind: ['轉換類：消耗 8 點 HP 換回 2 點 MP；不佔攻擊/治癒冷卻'],
    sk_elf_worldtree: ['被動。在「區域額外掉落」地圖(妖精森林周邊、眠龍洞穴 1~3 樓等)殺怪，粗糙米索莉塊/精靈玉/元素石的掉率由各 20% 提升為各 30%'],
    sk_elf_triple: ['需裝弓。一次射出 3 箭、每箭各自做命中與爆擊判定，但整招只消耗 1 支箭', '每命中 1 箭觸發一次熾炎天使弓的月光爆裂；發動時另觸發一次弓本身的疾風連射'],
    sk_elf_purify: ['精神 +3，持續 20 分鐘'],
    sk_elf_eleres: ['火/水/地/風抗性各 +10，持續 20 分鐘'],
    sk_elf_release: ['純即死魔法、本身不造成傷害', '只對帶「元素」標籤的非頭目怪生效；成功率吃等級差/怪魔防/魔法命中，上限 60%；失敗則毫無效果'],
    sk_elf_soul: ['轉換類：消耗 50 點 HP 換回 15 點 MP；不佔攻擊/治癒冷卻'],
    sk_elf_singleres: ['依你選定的妖精屬性，該單一屬性抗性 +50；持續 64 秒'],
    sk_elf_firewpn: ['近距離傷害 +3，持續 20 分鐘；需火屬性'],
    sk_elf_windshot: ['遠距離命中 +5，持續 20 分鐘；需風屬性'],
    sk_elf_winddash: ['迴避 +10，持續 20 分鐘；需風屬性'],
    sk_elf_earthguard: ['防禦(AC) -4，持續 20 分鐘；需地屬性'],
    sk_elf_groundtrap: ['全體緩速、不造成傷害；命中判定吃魔法命中與怪魔防，持續 30 秒；需地屬性'],
    sk_elf_watervital: ['期間「下一次瞬間治癒」(不含持續回復)治療量 ×2，觸發後 7 秒冷卻；全隊治療皆吃此加倍', '持續 64 秒，效果結束才能再施放'],
    sk_elf_magicerase: ['純狀態魔法、不造成傷害：使目標魔防減半，持續 16 秒；命中吃魔法命中與怪魔防'],
    sk_elf_summon: ['召喚屬性精靈(屬性＝你選的妖精屬性)，持續 60 分鐘、每 1 秒攻擊一次', '傷害與命中吃「魅力」不吃智力：每擊＝骰值(1~40)＋魅力×等級÷20，再扣魔防與傷害減免', '隻數：無精靈精通固定 1 隻；有精靈精通＝1＋魅力÷10(最多 7 隻，魅力以 60 封頂)'],
    sk_elf_dancefire: ['近距離傷害 +5，持續 20 分鐘；需火屬性'],
    sk_elf_stormeye: ['遠距離傷害 +2、遠距離命中 +2，持續 20 分鐘；需風屬性'],
    sk_elf_earthshield: ['持續 8 秒內完全免疫怪物「一般攻擊/連擊」傷害(對敵方魔法無效)；需地屬性'],
    sk_elf_lifespring: ['瞬間治療＝(骰值 5~250 ＋ 250)×(1＋3×魔法傷害÷16)，吃智力；需水屬性'],
    sk_elf_earthbless: ['防禦(AC) -7，全隊生效(你＋所有傭兵)；持續 20 分鐘；需地屬性'],
    sk_elf_summon2: ['召喚上級屬性精靈，持續 60 分鐘、每 1 秒攻擊一次', '較初階更強：每擊＝骰值(2~80)＋魅力×等級÷10(魅力加成基底加倍)、命中偏移 +20；隻數公式同初階精靈'],
    sk_elf_lifebless: ['全隊持續回復：每 3 秒回 1 次、共 6 次(約 18 秒)；每次＝(骰值 1~28 ＋ 28)×(1＋3×魔法傷害÷16)；需水屬性'],
    sk_elf_seal: ['純狀態魔法、不造成傷害：使目標「魔法封印」8 秒(該怪無法施法)；命中吃魔法命中與怪魔防'],
    sk_elf_blazewpn: ['近距離傷害 +5、近距離命中 +5，持續 20 分鐘；需火屬性'],
    sk_elf_flamesoul: ['持續內近距離「一般攻擊」的武器擲骰必定取最大值(等同每擊都吃重擊骰值)；持續 21 分 20 秒，效果結束才能再放；需火屬性'],
    sk_elf_stormshot: ['遠距離傷害 +6、遠距離命中 +3，持續 20 分鐘；需風屬性'],
    sk_elf_preciseshot: ['持續 64 秒內一般攻擊擲骰 1 由「必定未命中」變「必定命中」(命中率最高可達 100%)；效果結束才能再放；需風屬性'],
    sk_elf_steelguard: ['全隊受到傷害 -5%(乘算；你＋所有傭兵)，不再提供 AC；持續 20 分鐘；需地屬性'],
    sk_elf_attrfire: ['一般攻擊 30% 機率傷害 ×1.5(與黑暗妖精「燃燒鬥志」同效)；持續 5 分 20 秒，效果結束才能再放；需火屬性'],
    sk_elf_physboost: ['負重狀態下仍能自然回復 HP/MP(一般負重過重會停止恢復)；持續 16 分鐘，效果結束才能再放；需地屬性'],
    sk_elf_energyboost: ['效果同「體能激發」(負重下仍能自然回 HP/MP)；持續 16 分鐘，效果結束才能再放；需火屬性'],
    sk_elf_mirror: ['受到魔法「直接」傷害時，以「精神%」機率(每 1 點精神＝1%)對施法者反射等量、必中的固定傷害；持續 16 秒'],

    // ===== 黑暗妖精專屬(reqD) =====
    sk_dark_str: ['力量 +3，持續 16 分鐘'],
    sk_dark_mrup: ['魔防 +5，持續 16 分鐘'],
    sk_dark_stealth: ['100% 迴避「一次」物理攻擊，迴避後立即失效並進入 5 秒冷卻；持續 32 秒'],
    sk_dark_poison: ['一般攻擊命中 50% 機率使目標中毒：每秒造成「該次攻擊傷害的 60%」、持續 5 秒、最多 1 層(新毒更高才覆蓋刷新)；劇毒精通→100%、每秒 200%。持續 5 分 20 秒'],
    sk_dark_refine: ['被動。沉默洞穴周邊黑魔石②掉率 20%→30%、③ 10%→15%；並讓一般野外/地監也能掉黑魔石② 1%/③ 0.5%/④ 0.1%(無此被動則完全不掉，攻城區不掉)'],
    sk_dark_dex: ['敏捷 +3，持續 16 分鐘'],
    sk_dark_poisonres: ['持續內自身受到的中毒傷害減半；持續 5 分 20 秒'],
    sk_dark_burn: ['一般攻擊 30% 機率傷害 ×1.5；持續 5 分鐘'],
    sk_dark_walkhaste: ['攻速 +15%(與加速術等相乘疊加)；持續 16 分鐘'],
    sk_dark_fang: ['額外傷害 +5，持續 3 分 12 秒'],
    sk_dark_dodge: ['50% 機率迴避「必定命中」的傷害魔法，成功後失效並冷卻 5 秒；持續 32 秒'],
    sk_dark_crit: ['消耗 HP 上限 50%＋當前全部 MP；傷害＝一次「必中必重必爆、無視硬皮」的一般攻擊 ×(消耗 MP 佔上限%×10)(滿 MP→×10、半 MP→×5)；對「血盟」種族敵人再 ×2。HP 需高於一半才能施放'],
    sk_dark_erup: ['迴避 +12，持續 3 分 12 秒'],
    sk_dark_double: ['持「雙刀/鋼爪」時，一般攻擊 45 級起 10% 機率傷害 ×2、每 5 級再 +1%，可與燃燒鬥志疊加；非雙刀/鋼爪無效。持續 3 分 12 秒'],
    sk_dark_armorbreak: ['純狀態魔法、不造成傷害：使目標「破甲」8 秒，其後受到的傷害 ×1.58；命中吃魔法命中與怪魔防'],

    // ===== 騎士技術(reqK) =====
    sk_solid_shield: ['迴避 +15；需裝盾(武士刀可免盾)，消耗 HP30、MP5，持續 3 分鐘'],
    sk_reduction_armor: ['傷害減免 +等級÷10(無條件捨去)；持續 20 分鐘'],
    sk_shock_stun: ['需雙手(非弓)武器。做一次一般攻擊、傷害額外 +10；命中後 10% 機率觸發暈眩判定(再吃怪魔防)，成功則暈眩 6 秒'],
    sk_spike_armor: ['實際只提供近距離命中 +5(名稱雖為尖刺盔甲，並無反傷效果)；持續 20 分鐘'],
    sk_counter_barrier: ['讓「雙手武器」也能發動反擊(原本僅單手劍原生反擊/武士刀居合)；且原生反擊與居合的最終傷害 ×2；持續 64 秒，效果結束才再施放'],

    // ===== 幻術士 記憶水晶法術(reqI) =====
    sk_illu_ogre: ['增益：額外傷害 +4、額外命中 +4，持續 64 秒，消耗 20MP', '只有選「幻術精通」才多召歐吉幻象一起打：每 2 秒一次近戰，傷害 3~60＋(智力÷5)×(1＋等級÷10)，吃智力。沒精通就只有上面那個被動加成'],
    sk_illu_confuse: ['造成 2~22 無屬性魔法傷害(吃智力)，消耗 15MP＋10HP', '附「混亂」8 秒：以魔法命中判定(看等級差、魔法命中、怪魔防，非必中)。命中後該怪防禦(AC)+5 更容易被打、且其攻擊傷害 -10', '目標已在混亂中就不會重複施放(不耗資源)'],
    sk_illu_crush: ['以物理公式算底傷(武器骰＋近或遠距離傷害＋強化值)，整體再 ×(1＋魔法傷害÷16)，不計武器特效', '屬魔法技能→必定命中、無視目標防禦與硬皮。同時吃武器與智力，MP 只要 5，非常省'],
    sk_illu_mirror: ['迴避 +25，持續 20 分鐘，消耗 10MP'],
    sk_illu_focus: ['MP 自然恢復 +4，持續 10 分鐘，消耗 30MP'],
    sk_illu_lich: ['增益：魔法傷害 +2，持續 64 秒，消耗 20MP', '只有「幻術精通」才多召巫妖幻象：每 3 秒一次魔法，傷害 3~60＋(智力÷5)×(1＋等級÷10)，必定命中但受怪魔防減免；吃智力'],
    sk_illu_mindbreak: ['基礎傷害＝本次消耗的 MP 量(固定為最大 MP 的 5%)，再 ×(1＋魔法傷害÷16)並受怪魔防減免，無屬性', '最大 MP 越高、這招越痛。不消耗 HP'],
    sk_illu_cube_quake: ['召出地裂立方：持續 20 秒，每 4 秒對全場敵人各判定一次「緩速」(以魔法命中判定，持續 4 秒)', '自身地抗 +30，消耗 35MP'],
    sk_illu_skullbreak: ['只能對不死系怪。先做即死判定(最高約 60%，對頭目無效)', '即死失敗才造成傷害＝粉碎能量公式再 +20 固定值(物理、無視防禦與硬皮)。消耗 30MP'],
    sk_illu_fantasy: ['造成 5~60 無屬性魔法傷害(吃智力，三階魔攻係數)，消耗 30MP＋25HP', '附「沉睡」8 秒：以魔法命中判定，且頭目免疫沉睡'],
    sk_illu_golem: ['增益：全隊防禦(AC) -10，持續 64 秒，消耗 30MP＋25HP', '只有「幻術精通」才多召鑽石高崙幻象：每 1 秒一次近戰，傷害 2~40＋(智力÷5)×(1＋等級÷5)，命中高，10% 機率觸發冰矛圍籬；吃智力'],
    sk_illu_cube_shock: ['召出衝擊立方：持續 20 秒，每 5 秒對當前目標施加「魔抗減半」4 秒', '自身風抗 +30，消耗 55MP'],
    sk_illu_endure: ['傷害減免 +2，持續 10 分鐘，消耗 25MP'],
    sk_illu_avatar: ['額外傷害 +10、受到傷害 -3%，兩者都全隊生效，持續 64 秒，消耗 50MP'],
    sk_illu_panic: ['純異常(無傷害)，附「恐慌」64 秒(很長)：該怪防禦(AC)+5、攻擊傷害 -10', '以魔法命中判定，非頭目免疫類。已在恐慌中不重複施放。消耗 30MP＋30HP'],
    sk_illu_insight: ['力量/敏捷/體質/智力/精神 各 +1，持續約 10.7 分鐘，消耗 60MP'],
    sk_illu_cube_burn: ['召出燃燒立方：持續 20 秒，每 4 秒對全場敵人各造成 1~25 火傷', '自身火抗 +30。立方傷害是固定骰值，不吃智力/魔法傷害，只受武器強化(+11 以上)倍率與脆弱影響；仍會被怪魔防與傷害減免削減。消耗 30MP'],
    sk_illu_cube_harmony: ['召出和諧立方：持續 20 秒，每 1 秒對當前目標造成 1~25 火傷、同時回全隊 5MP', '消耗 25HP、不耗 MP'],
    sk_illu_pain: ['增益 64 秒(消耗 40HP、不耗 MP)：期間每次被攻擊(一般或技能)都反射「等同你所受傷害」的無屬性魔法傷害給攻擊者(100% 觸發，吃脆弱)'],

    // ===== 龍騎士 龍魔法(reqDk) =====
    sk_dragon_armor: ['傷害減免 +5，持續 30 分鐘，消耗 12HP', '效果結束後才能再施放(不刷新)'],
    sk_dragon_flameslash: ['須裝近距離武器。消耗 6HP，讓「下一次近戰一般攻擊」額外傷害 +7 並轉為火屬性(用掉即消失)，最長維持 60 秒'],
    sk_dragon_guardbreak: ['無直接傷害。消耗 20HP，10% 機率讓目標陷入「護衛毀滅」32 秒→該怪防禦(AC)+10，你更容易命中它', '固定機率、不吃魔防，對頭目也有效。目標已中就不重複'],
    sk_dragon_lavaspit: ['傷害吃智力(魔法傷害)，完全不吃力量——純力量龍騎放這招很弱', '對全場敵人各造成 5~35(5 顆 1~7 骰)火屬性魔法傷害(每顆骰再 ×一階魔攻係數)，受怪魔防與火屬性剋制影響。消耗 10HP', '龍血套裝 5/5 讓這類消耗 HP 技傷害 +20%；龍血精通則 HP 消耗減半'],
    sk_dragon_awaken_antares: ['增益 10 分鐘，消耗 20MP＋10HP：防禦(AC) -8、免疫中毒與麻痺、最大 HP +(2×等級)、攻速 +20%(覺醒精通改 +50%)', '三種覺醒同時只能維持一種(覺醒精通例外)'],
    sk_dragon_bloodlust: ['攻速 +15%(與加速/覺醒/變身相乘疊加)，持續 5 分鐘，消耗 30HP，不刷新'],
    sk_dragon_slaughter: ['立即額外進行 3 次近距離一般攻擊(各自判定命中/爆擊，吃力量與武器)，須裝近距離武器，消耗 16HP', '目標每層「弱點曝光」讓這三刀每一擊都 +10 傷害(觸發後消耗該層；弱點精通不消耗)', '龍血套裝 5/5 讓傷害 +20%'],
    sk_dragon_terror: ['無直接傷害。消耗 12HP，10% 機率讓目標陷入「恐懼」16 秒→該怪 90% 機率攻擊落空', '固定機率、對頭目也有效。目標已中就不重複'],
    sk_dragon_lavabolt: ['傷害吃智力(魔法傷害)，完全不吃力量——純力量龍騎放這招很弱', '對單體造成 10~80(10 顆 1~8 骰)火屬性魔法傷害(每顆骰再 ×二階魔攻係數)，受怪魔防與火屬性剋制影響。消耗 16HP', '龍血套裝 5/5 傷害 +20%'],
    sk_dragon_awaken_falion: ['增益 10 分鐘，消耗 30MP＋20HP：魔防 +15%、火/水/地/風四屬性抗性各 +15、攻速 +20%(覺醒精通 +50%)', '覺醒三選一，同時只能開一種'],
    sk_dragon_deadlybody: ['增益 5 分鐘，消耗 50HP：期間每次被攻擊(一般或技能)有 23% 機率反射等量傷害給攻擊者(無屬性魔法，吃脆弱)，不刷新'],
    sk_dragon_deathlightning: ['傷害吃智力(魔法傷害)，不吃力量', '對全場敵人各造成 6~48(6 顆 1~8 骰)風屬性魔法傷害(三階)，並附「暈眩」6 秒(以魔法命中判定，頭目免疫暈眩)。消耗 35HP'],
    sk_dragon_reaper: ['無直接傷害。消耗 20HP，50% 機率讓目標陷入「死神」32 秒→該怪攻擊傷害 -20', '固定機率、不吃魔防，對頭目也有效。目標已中就不重複'],
    sk_dragon_awaken_baraka: ['增益 10 分鐘，消耗 50MP＋30HP：力量/體質/敏捷/智力/精神 各 +3、額外命中 +5、攻速 +20%(覺醒精通 +50%)', '覺醒三選一，同時只能開一種'],

    // ===== 戰士技能印記(reqW) =====
    sk_warrior_dualaxe: ['副手那一擊是「完整、獨立的一般攻擊」：自己擲命中(可落空)、自己算傷害，並同樣吃狂暴 5% 兩倍', '副手不會重複觸發主手的出血、弱點曝光等武器特效(只有主手那刀會)', '搭配巨斧精通時，主手與副手可改持雙手鈍器(雙手鈍器單手化)'],
    sk_warrior_crush: ['只加成「近距離傷害」，遠程武器打不到這份加成', '公式：+2，等級 45 起每再升 1 級多 +1(等級 50＝+8、等級 100＝+58)'],
    sk_warrior_armorbody: ['傷害減免＝無條件捨去[(10 − 防禦(AC))÷10]。本作防禦(AC)是負值，越低(越強)減免越高：防禦(AC) -40 → 減免 +5', '防禦(AC)為正(很弱)時算出 0、沒有效果', '學堅韌精通後除數改成 5(減免加倍)：防禦(AC) -40 → 減免 +10'],
    sk_warrior_berserk: ['每個攻擊來源各自獨立擲 5%：主手一次、迅猛雙斧副手一次、反彈精通的追擊也各擲一次'],
    sk_warrior_titan_rock: ['HP 低於 40% 時，受一般攻擊反射相同傷害給攻擊你的那隻怪；反射量還會吃目標的脆弱/破甲加成', '學反彈精通後：觸發門檻放寬到 HP 80%，且每次反射會額外對攻擊者發動一次普通攻擊(有雙持則主副手各一次)'],
    sk_warrior_titan_magic: ['HP 低於 40% 時，受「技能」攻擊反射相同傷害給施法的那隻怪；反射量吃目標脆弱/破甲加成', '學反彈精通後：門檻放寬到 HP 80%，且觸發時額外對攻擊者發動一次普通攻擊'],
    sk_warrior_titan_bullet: ['HP 低於 40% 時，迴避 +50(即時判定，不進屬性面板)', '學反彈精通後門檻放寬到 HP 80%'],
    sk_warrior_throwaxe: ['一次性：下一次「近戰」一般攻擊命中時附加出血後即消耗，出血強度依那一擊的傷害、最多疊 5 層', '須裝單手或雙手鈍器才施放得出來；持續 60 秒(效果結束才能再施放)', '學雙斧精通後：施放免消耗魔力，且出血每層額外 +10% 傷害'],
    sk_warrior_roar: ['對全場敵人造成固定 50＋(等級 − 30，未滿 30 級以 0 計)點傷害，無視魔防、減傷、屬性剋制', '但仍會吃目標的脆弱/精準目標/破甲加成。消耗魔力 5'],
    sk_warrior_endurance: ['HP 上限 +(等級÷2)%(等級 60＝+30%)。持續 50 分鐘，效果結束才能再施放'],
    sk_warrior_outlaw: ['把一般攻擊的命中率下限鎖在約 50%：即使面對等級遠高於你的怪也保底能打中', '持續 60 秒'],

    // ===== 王族魔法(reqRoy) =====
    sk_royal_precise: ['讓全場所有敵人受到的每一種傷害(物理、魔法、固定傷害都算)增加 [1＋(等級÷15)]%(等級 60＝5%、等級 150＝11%)', '學血盟精通後除數 15→10(更強，等級 60＝7%)', '場上只要有任一持此增益的傭兵，全隊都吃得到；持續 16 秒，效果結束才能再施放'],
    sk_royal_callally: ['所有上場傭兵立即各自發動一次額外的一般攻擊(傷害就是各傭兵自己的普攻)；本身不造成傷害', '沒有傭兵或沒有目標時放不出來。消耗魔力 30，學血盟精通後減半為 15'],
    sk_royal_bravewill: ['10% 機率讓一般攻擊傷害 ×1.5(學王族劍術精通後提高到 20%)。持續約 10 分 40 秒'],
    sk_royal_kingguard: ['魔防 +10；20% 機率抵抗暈眩', '抗暈與裝備的抗暈是「取較高者」、不疊加'],

    // ===== 魔法頭盔技能(頭盔授予,持有即用,不限等級) =====
    sk_helm_heal1: ['治療量＝(1~20＋20)×(1＋3×魔法傷害÷16)，會隨智力成長。消耗魔力 2'],
    sk_helm_heal2: ['治療量＝(1~50＋50)×(1＋3×魔法傷害÷16)，隨智力成長。消耗魔力 7'],
    sk_helm_dex1: ['敏捷 +5，持續 20 分鐘。與法師的同名魔法擇一生效(頭盔版優先、不疊加)'],
    sk_helm_dex2: ['加速：攻擊速度 +33%(與其他加速相乘疊加)，持續 20 分鐘。消耗魔力 20'],
    sk_helm_str1: ['額外傷害 +2，持續 30 分鐘。與法師的「擬似魔法武器」擇一生效(頭盔版優先、不疊加)'],
    sk_helm_str2: ['沒有實際的戰鬥數值效果(沿用原版「看穿隱身」，本作沒有隱身怪)。持續 180 秒'],
    sk_helm_str3: ['力量 +5，持續 20 分鐘。消耗魔力 25']
  };
