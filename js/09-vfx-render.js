...
// 🗡️ 本質是「立即多打幾次一般攻擊」的技能(屠宰者=近戰連斬、三重矢=連射)：施放時改播「攻擊動畫連播 hits 次」而非施法動作。
const _PM_ATTACK_SKILLS = { sk_dragon_slaughter: true, sk_elf_triple: true };   // 玩家端：以技能 id 判斷·次數讀 DB.skills[id].hits
...
if (typeof castSkill === 'function' && !castSkill._vfxWrapped) {
    ...
    castSkill = function (skId) {
        ...
        if (r) { try {   // 🧝 v3.0.105 施法動作只在「實際施放成功(r)」才播（修：自動恢復/維持技即使沒真的施放·仍每 tick 觸發施法動畫→sprite 卡在施法姿勢）
            if (_PM_ATTACK_SKILLS[skId] && typeof _playerMorphAttackBurst === 'function') _playerMorphAttackBurst(skId);   // 🗡️ 屠宰者/三重矢：改播攻擊動畫連擊
