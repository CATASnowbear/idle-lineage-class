...
(function () {
  ...
  var SLICE_MS         = 28;                    // 每段最多跑這麼久就 await raf 讓出一次。進度遮罩只在「讓出時」才重繪、期間頁面凍結,
                                                //   故這個值＝畫面更新間隔:用接近一個影格(~16ms)的小值,結算過程才順、不會一卡一卡。
                                                //   (放大可減少等影格開銷、加快結算,但畫面會頓——250ms 使用者反映不流暢,故維持小值、優先流暢。)
